...
class TwoFAHelper
{
     ...
     * @var string
     */
    private static $SecretFile = '/etc/brainy/data/2FA/users.dat';
    ...
    {
        $data = @json_decode(file_get_contents(self::$SecretFile), true);
        $output = shell_exec("/usr/bin/php72/bin/php -q /etc/brainy/2FA/getqrcode.php username=".escapeshellarg($_SESSION["user"])." secret=".escapeshellarg($data[$_SESSION["user"]]["secret"]));
        return $output;
    }
